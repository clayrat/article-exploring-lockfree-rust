...
fn parse_bytes(b: Box<[u8]>) -> Option<Payload> {
    let start = OffsetDateTime::now_utc().nanosecond();
    let mut d = OffsetDateTime::now_utc().nanosecond() as f64 / start as f64;
    for _ in 0..10_000 {
        d *= 1.00001;
    }
    ...
    where FN: Fn(Box<[u8]>) -> Option<Payload>
{
    ...
    let elapsed = OffsetDateTime::now_utc().nanosecond() - start;
